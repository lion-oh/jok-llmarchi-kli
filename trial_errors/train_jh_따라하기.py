...
def define_argparser(
    ...
    p.add_argument("--max_length", default=4000)
...
if __name__ == "__main__":
    ...
    train_dataset = Dataset.from_dict(lambda_unsqeeze(train_dataset)).map(lambda x: x, batched=True)
    valid_dataset = Dataset.from_dict(lambda_unsqeeze(valid_dataset)).map(lambda x: x, batched=True)
